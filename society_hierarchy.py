...
class Citizen:
    ...
    def become_subordinate_to(self, superior: Optional[Citizen]) -> None:
        ...
        if superior is None:
            ...
            self._superior.add_subordinate(self)
            return None
    ...
    def get_all_subordinates(self) -> List[Citizen]:
        ...
        total = self._subordinates
        for i in self._subordinates:
            total = merge(total, i.get_all_subordinates())
        return total
    ...
    def get_society_head(self) -> Citizen:
        ...
        if self._superior is None:
            ...
            head = self._superior.get_society_head()
            return head
    ...
    def get_closest_common_superior(self, cid: int) -> Citizen:
        ...
        if self.get_citizen(cid) is not None:
            ...
            head = self._superior.get_closest_common_superior(cid)
            return head
    ...
    def get_highest_rated_subordinate(self) -> Citizen:
        ...
        total = self.get_direct_subordinates()
        curr = total[0]
        for i in total:
            if i.rating > curr.rating:
                curr = i
        return curr
...
class Society:
    ...
    def add_citizen(self, citizen: Citizen, superior_id: int = None) -> None:
        ...
        if superior_id is None:
            sup = citizen.get_direct_subordinates()
            if not sup == []:
                for i in sup:
            ...
            name = self._head.get_citizen(superior_id)
            citizen.become_subordinate_to(name)
    ...
    def change_citizen_type(self, cid: int,
        ...
        citi = self.get_citizen(cid)
        if isinstance(citi, DistrictLeader):
            regular_citi = Citizen(citi.cid, citi.manufacturer,
                                   citi.model_year, citi.job, citi.rating)
            if citi != self._head:
                sup = citi.get_superior()
                sup.remove_subordinate(citi.cid)
                regular_citi.become_subordinate_to(sup)
            else:
                self._head = regular_citi
            for i in citi.get_direct_subordinates():
                regular_citi.add_subordinate(i)
            return regular_citi
        else:
            dis_citi = DistrictLeader(citi.cid, citi.manufacturer,
                                      citi.model_year, citi.job, citi.rating,
                                      district_name)
            if citi != self._head:
                sup = citi.get_superior()
                sup.remove_subordinate(citi.cid)
                dis_citi.become_subordinate_to(sup)
            else:
                self._head = dis_citi
            for i in citi.get_direct_subordinates():
                dis_citi.add_subordinate(i)
            return dis_citi
    ...
    def _swap_up(self, citizen: Citizen) -> Citizen:
        ...
        if isinstance(citizen.get_superior(), DistrictLeader):
            ...
            ss = s.get_superior()
            cit = DistrictLeader(citizen.cid, citizen.manufacturer,
                                 citizen.model_year, s.job, citizen.rating,
                                 s.get_district_name())
            sup = Citizen(s.cid, s.manufacturer, s.model_year, citizen.job,
                          s.rating)
            for i in citizen.get_direct_subordinates():
                citizen.remove_subordinate(i.cid)
                sup.add_subordinate(i)
            cit.add_subordinate(sup)
            for j in s.get_direct_subordinates():
                s.remove_subordinate(j.cid)
                cit.add_subordinate(j)
            cit.remove_subordinate(citizen.cid)
            if ss is not None:
                ss.add_subordinate(cit)
                ss.remove_subordinate(s.cid)
            else:
                self._head = cit
            return cit
        else:
            s = citizen.get_superior()
            ss = s.get_superior()
            cit = Citizen(citizen.cid, citizen.manufacturer, citizen.model_year,
                          s.job, citizen.rating)
            sup = Citizen(s.cid, s.manufacturer, s.model_year, citizen.job,
                          s.rating)
            for i in citizen.get_direct_subordinates():
                citizen.remove_subordinate(i.cid)
    ...
    def promote_citizen(self, cid: int) -> None:
        ...
        """
        curr = self.get_citizen(cid)
        sup = curr.get_superior()
        if curr == self._head:
            return None
        if curr.rating <= sup.rating:
            return None
    ...
    def delete_citizen(self, cid: int) -> None:
        ...
        if sup is not None:
            for sub in curr.get_direct_subordinates():
                sup.add_subordinate(sub)
            sup.remove_subordinate(cid)
        else:
            sub = curr.get_direct_subordinates()
            if not sub:
                self._head = None
            else:
                new = curr.get_highest_rated_subordinate()
                new.set_superior(None)
                sub.remove(new)
                self.set_head(new)
                for i in sub:
                    new.add_subordinate(i)
